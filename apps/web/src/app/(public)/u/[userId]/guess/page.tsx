...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${displayName}'s Guess profile`;
...
export default async function PublicGuessProfilePage({ params }: PageProps) {
  const { userId } = await params;
  return <PublicProfileClient userId={userId} brand="guess" scope="guess" />;
}
